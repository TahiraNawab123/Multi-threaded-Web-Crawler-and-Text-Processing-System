...
static int local_has_palindrome(LocalStats *s, const char *w)
{
    for (int i = 0; i < s->pal_count; i++)
        if (strcmp(s->palindromes[i], w) == 0) return 1;
    return 0;
}

static void local_add_palindrome(LocalStats *s, const char *w)
{
    if (local_has_palindrome(s, w)) return;
    if (s->pal_count == s->pal_capacity) {
        s->pal_capacity *= 2;
        s->palindromes = realloc(s->palindromes,
...
static int local_word_insert(LocalStats *s, const char *word)
{
    ...
    for (struct WordNode *n = s->word_table[idx]; n; n = n->next)
        if (strcmp(n->word, word) == 0) return 0;
...
void process_chunk(LocalStats *s, const char *text)
{
    ...
    while (token) {
        ...
        int  ci = 0;
        const char *tp = token;
        while (*tp && !isalpha((unsigned char)*tp)) tp++;
        while (*tp && isalpha((unsigned char)*tp) && ci < MAX_WORD_LEN - 1)
            clean[ci++] = (char)tolower((unsigned char)*tp++);
        clean[ci] = '\0';

        if (ci >= 1) {
            s->total_words++;
            if (local_word_insert(s, clean)) s->unique_word_count++;
...
{
    if (!s) return;
    for (int i = 0; i < s->pal_count; i++) free(s->palindromes[i]);
    free(s->palindromes);
