...
int main(int argc, char *argv[])
{
    ...
    if (total_threads < 2) { fprintf(stderr, "total_threads >= 2\n"); return 1; }
    if (chunk_size    < 1) { fprintf(stderr, "chunk_size >= 1\n");    return 1; }

    int D, P;
    if (argc == 5) {
        D = atoi(argv[4]);
        if (D < 1) D = 1;
        if (D > total_threads - 1) D = total_threads - 1;
    } else {
        D = total_threads / 3;
        if (D < 1) D = 1;
    }
    ...
    for (int i = 0; i < P; i++) {
        ...
        par_args[i]  = (ParserArgs){chunk_queue, locals[i], &par_acc, &par_lock};
