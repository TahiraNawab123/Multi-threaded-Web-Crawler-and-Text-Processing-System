...
#include <curl/curl.h>
    ...
    // Buffer used to store downloaded data 
typedef struct {
    char *data;
    size_t size;
} 
...
// Used to keep track of downloader threads 
...
// Read URLs from file and put them in the queue
void *url_reader_thread(void *arg)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        if (len == 0)
            continue;
        // Create URL item and add it to the queue
...
// Download pages and create chunks for parser threads
void *downloader_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        while (tok)
        {
            ...
            while (used + tlen + 2 > cap)
            {
                cap *= 2;
                cbuf = realloc(cbuf, cap);
            }
            if (used > 0)
                cbuf[used++] = ' ';
            ...
            cbuf[used] = '\0';
            wc++;
            // Chunk is ready, send it to parser queue
            if (wc == a->chunk_size)
            {
    ...
    *a->dl_time_acc += elapsed;
    ...
    // Close chunk queue after the last downloader finishes
